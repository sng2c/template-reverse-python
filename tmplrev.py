...
class Chunk:
    ...
    def __str__(self):
        return " ".join((str(item) for item in self.left if not isinstance(item, Symbol))) + " ... " + "".join(
            (str(item) for item in self.right if not isinstance(item, Symbol)))
...
class TestMain(unittest.TestCase):
    ...
    def test_diff(self):
        ...
        self.assertEqual(tuple(partition(d, 2, 1)),
                         ((BOF(), 'Hello'), ('Hello', 'I'), ('I', ANY()), (ANY(), 'khs'), ('khs', EOF())))
        ...
        self.assertEqual(tuple(partition(partition_by(d, ANY()), 3, 2)),
                         (([BOF()], ANY(), ['there,']), (['there,'], ANY(), ['was']), (['was'], ANY(), ['there']),
                          (['there'], ANY(), [EOF()])))
    ...
    def test_detect(self):
        ...
        d = detect("Hello there, I was been there aaa".split(),
                   "Hello there, he was in there khs".split())
        self.assertEqual(tuple(d),
                         (([BOF(), 'Hello', 'there,'], ANY(), ['was']), (['was'], ANY(), ['there']),
                          (['there'], ANY(), [EOF()])))
